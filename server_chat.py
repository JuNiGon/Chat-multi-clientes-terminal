...
clients_lock = threading.Lock() # para proteger acesso concorrente a estruturas globais
clients = {} # conn -> {"nick": str, "room": str or None}
rooms = {} # room -> set(conns)

def safe_send(conn, text): #encapsula envio de texto. Acrescenta \n, codifica e usa sendall
    try:
        conn.sendall((text + "\n").encode(ENC))
    except Exception:
        # falha ao enviar (cliente possivelmente desconectado)
        remove_client(conn) # chama remove_client para limpeza
    ...
    try:
        while True:
            ...
            if text.startswith("/nick "):
                ...
                # mensagem normal => enviar para sala atual
                with clients_lock:
